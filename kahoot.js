...
http.listen(process.env.PORT, function() {
	console.log(
		`Kahoot auto answer started. Running on PORT ${process.env.PORT}`
	);
});

function LoadConfig() {
	let file = fs.readFileSync(
		"/var/www/charleyw/api.charleyw.com/Configs/Kahoot_Credentials.json"
	);
...
function LoadKeys() {
	let file = fs.readFileSync(
		"/var/www/charleyw/api.charleyw.com/Configs/Kahoot_Keys.json"
	);
	return JSON.parse(file);
}
...
function kahoot(request) {
	try {
		...
		client.on("quizStart", quiz => {
			...
			let gennedURL = `https://create.kahoot.it/search?filter=1&query=${quiz.name}&tags=${quiz.name}`;

			const URL = gennedURL.replace(/\s-\s/g, "-");

			console.log(URL);

			const USERNAME = quiz.client.name;
			let data = JSON.parse(
				fs.readFileSync(
					"/var/www/charleyw/api.charleyw.com/Configs/Kahoot.json"
				)
			);
			data.push({
				name: USERNAME,
			...
			(async () => {
				try {
										...
										if (noAnswer) {
											try {
												if (
													...
													let AnswerText = await choice.$eval(
														"span:nth-child(2)",
														e => e.innerHTML
													);
													AnswersTextTemp.push(AnswerText);
					...
					client.on("questionEnd", questionEnd => {
						for (let i = 0; i < Kahoots.length; i++) {
							if (
								Kahoots[i].AnswersText[questionEnd.question.index] !=
